...
function unpackVoiceSelection(value) {
  ...
  if (clean.startsWith("profile:")) return { voice_profile_id: clean.slice(8), voice_sample_id: "" }
  if (clean.startsWith("sample:")) return { voice_profile_id: "", voice_sample_id: clean.slice(7) }
  return { voice_profile_id: "", voice_sample_id: clean }
}

function findVoiceMeta(ref, profiles, samples) {
  ...
  if (clean.startsWith("profile:")) {
    ...
    return {
      ...
      variationLabel: item.voice_variation_policy?.allow_variants === false ? "sem variações" : `pitch ${item.voice_variation_policy?.pitch_range_min ?? "-"}..${item.voice_variation_policy?.pitch_range_max ?? "-"}`,
    }
  }
  if (clean.startsWith("sample:")) {
    ...
    return {
      ...
      variationLabel: "sample original",
    }
  }
  return null
}
...
function VoiceOptionLabel({ voice }) {
  const owner = voice.owner_person_name || voice.owner_person_id || "-"
  const credit = voice.credited_name || voice.owner_person_name || voice.display_name || "-"
  return `${voice.display_name}${voice.language ? ` — ${voice.language}` : ""} · dono: ${owner} · crédito: ${credit}`
}
...
export default function AudioCastPanel({ user }) {
  ...
  useEffect(() => { loadProjectsAndVoices() }, [])
  useEffect(() => { if (selectedProjectId) loadCast(selectedProjectId) }, [selectedProjectId])

  async function loadProjectsAndVoices() {
    const [projectsRes, voiceSamplesRes, voiceProfilesRes] = await Promise.all([listProjects(user), listVoiceSamples(), listVoiceProfiles()])
  ...
  function updateRow(index, patch) { setRows((current) => current.map((row, i) => i === index ? { ...row, ...patch } : row)) }

  async function handleSave() {
    if (!selectedProjectId) return
    try {
      await saveAudioCast(selectedProjectId, {
        narrator: { ...unpackVoiceSelection(narratorVoiceRef), provider: "xtts" },
        characters: rows.map((row) => ({ name: row.name, ...unpackVoiceSelection(row.voice_ref), provider: row.provider || "xtts", notes: row.notes || "" }))
      })
  ...
  async function handlePreview(characterName) {
    if (!selectedProjectId) return
    try {
      const res = await previewAudioCast(selectedProjectId, { character_name: characterName, provider: previewProvider, text: previewText || `Olá. Este é um teste da personagem ${characterName}.` })
  ...
  return (
    <Card title="Audio Cast por Personagem">
      ...
      <select value={previewProvider} onChange={(e) => setPreviewProvider(e.target.value)} style={{ padding: 12, borderRadius: 12, border: "1px solid #d1d5db", outline: "none" }}>
      ...
      <textarea value={previewText} onChange={(e) => setPreviewText(e.target.value)} rows={3} style={{ width: "100%", padding: 10, borderRadius: 10, border: "1px solid #d1d5db", outline: "none", resize: "vertical" }} />

      <label>Voz do narrador</label>
      <select value={narratorVoiceRef} onChange={(e) => setNarratorVoiceRef(e.target.value)} style={{ padding: 12, borderRadius: 12, border: "1px solid #d1d5db", outline: "none" }}>
        <option value="">Sem voz fixa</option>
        <optgroup label="Voice profiles aprovados">
          {approvedVoiceProfiles.map((voice) => <option key={`profile-${voice.id}`} value={`profile:${voice.id}`}>{VoiceOptionLabel({ voice })}</option>)}
        </optgroup>
        <optgroup label="Voice samples antigas">
          {voiceSamples.map((voice) => <option key={`sample-${voice.id}`} value={`sample:${voice.id}`}>{voice.name} {voice.language ? `— ${voice.language}` : ""}</option>)}
        </optgroup>
      </select>
      ...
      <div style={{ padding: 12, borderRadius: 12, border: "1px solid #e5e7eb", background: "rgba(255,255,255,0.55)", display: "grid", gap: 10 }}>
        ...
        {rows.map((row, index) => {
          const rowMeta = findVoiceMeta(row.voice_ref, approvedVoiceProfiles, voiceSamples)
          return (
            <div key={`${row.name}-${index}`} style={{ border: "1px solid #d1d5db", borderRadius: 12, padding: 10, display: "grid", gap: 8, background: "#fff" }}>
              <div><strong>{row.name}</strong></div>
              <select value={row.voice_ref} onChange={(e) => updateRow(index, { voice_ref: e.target.value })} style={{ padding: 10, borderRadius: 10, border: "1px solid #d1d5db", outline: "none" }}>
                <option value="">Usar narrador/default</option>
                <optgroup label="Voice profiles aprovados">
                  {approvedVoiceProfiles.map((voice) => <option key={`row-profile-${voice.id}`} value={`profile:${voice.id}`}>{VoiceOptionLabel({ voice })}</option>)}
                </optgroup>
                <optgroup label="Voice samples antigas">
                  {voiceSamples.map((voice) => <option key={`row-sample-${voice.id}`} value={`sample:${voice.id}`}>{voice.name} {voice.language ? `— ${voice.language}` : ""}</option>)}
                </optgroup>
              </select>
              ...
              <button onClick={() => handlePreview(row.name)} style={{ padding: "8px 10px", borderRadius: 10, border: "none", background: "#0369a1", color: "#fff", cursor: "pointer" }}>Ouvir preview</button>
            </div>
          )
        })}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button onClick={() => handlePreview("Narrador")} style={{ padding: "10px 12px", borderRadius: 12, border: "none", background: "#0f766e", color: "#fff", fontWeight: 700, cursor: "pointer" }}>Ouvir narrador</button>
        <button onClick={handleSave} style={{ padding: "10px 12px", borderRadius: 12, border: "none", background: "#7c3aed", color: "#fff", fontWeight: 700, cursor: "pointer" }}>Guardar casting</button>
      </div>
