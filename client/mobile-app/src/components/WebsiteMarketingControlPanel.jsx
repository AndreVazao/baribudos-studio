...
const PUBLIC_STATES = ["private", "internal_review", "teaser_ready", "prelaunch_public", "launch_ready", "published", "archived"]
...
function ActionButton({ children, onClick, disabled = false, tone = "primary" }) {
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      style={{ padding: "10px 12px", borderRadius: 12, border: palette.border || "none", background: disabled ? "#94a3b8" : palette.background, color: palette.color, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer" }}
    >
      {children}
    </button>
  )
...
export default function WebsiteMarketingControlPanel({ user }) {
  ...
  async function refreshProject(projectId) {
    ...
    try {
      ...
      setMarketing({ ...EMPTY_MARKETING, ...(nextCommercial?.website_marketing || nextCommercial?.marketing || {}) })
  ...
  function autofillFromProject() {
    const images = assetCandidates.filter(looksLikeImage)
    const videos = assetCandidates.filter(looksLikeVideo)
    updateMarketing({
  ...
  async function saveMarketing(nextMarketing, successMessage) {
  ...
  return (
    <Card title="Website Marketing Control" extra={<ActionButton onClick={() => refreshProject(selectedProjectId)} disabled={busy || !selectedProjectId} tone="secondary">{busy ? "A atualizar..." : "Atualizar"}</ActionButton>}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <ActionButton onClick={() => saveMarketing(marketing, "Controlo de marketing do Website guardado.")} disabled={busy || !selectedProjectId}>Guardar controlo marketing</ActionButton>
      ...
      <input value={marketing.teaser_headline || ""} onChange={(e) => updateMarketing({ teaser_headline: e.target.value })} style={{ padding: 12, borderRadius: 12, border: "1px solid #d1d5db", outline: "none" }} />
      ...
      <input value={marketing.teaser_release_label || ""} onChange={(e) => updateMarketing({ teaser_release_label: e.target.value })} style={{ padding: 12, borderRadius: 12, border: "1px solid #d1d5db", outline: "none" }} />
