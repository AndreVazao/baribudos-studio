...
function getSalesReadiness(project, marketing, websiteSync) {
  const checks = [
    { key: "headline", label: "Headline forte", ok: !!String(marketing?.teaser_headline || "").trim() },
    { key: "subtitle", label: "Subtítulo claro", ok: !!String(marketing?.teaser_subtitle || "").trim() },
    { key: "cta", label: "CTA definido", ok: !!String(marketing?.teaser_cta_label || "").trim() },
    { key: "cover", label: "Cover principal", ok: !!String(marketing?.teaser_cover_url || project?.cover_image || "").trim() },
    { key: "gallery", label: "Galeria mínima", ok: Array.isArray(marketing?.teaser_gallery) && marketing.teaser_gallery.length > 0 },
    { key: "state", label: "Estado público coerente", ok: ["teaser_ready", "prelaunch_public", "launch_ready", "published"].includes(marketing?.public_state) },
    { key: "project-ready", label: "Projeto pronto para publicação", ok: !!project?.ready_for_publish },
    { key: "website-sync", label: "Sync Website conhecido", ok: !!(websiteSync?.publication_id || websiteSync?.published_at || websiteSync?.last_revalidate_at) },
  ]
...
const isImage = (value) => /\.(png|jpg|jpeg|webp|gif|bmp|svg)$/i.test(String(value || ""))
const isVideo = (value) => /\.(mp4|webm|mov|m4v)$/i.test(String(value || ""))
...
export default function WebsiteMarketingControlPanel({ user }) {
  ...
  const setPatch = (patch) => setMarketing((current) => ({ ...current, ...patch }))
  const useCover = (value) => setPatch({ teaser_cover_url: value })
  const useTrailer = (value) => setPatch({ teaser_trailer_url: value })
  const addGallery = (value) => setPatch({ teaser_gallery: Array.from(new Set([...(marketing.teaser_gallery || []), value])) })
  ...
  function applyPreset(mode) { setMarketing(buildPreset(mode)) }
  ...
  async function promoteLaunch() {
    ...
    try {
      await updateProjectCommercial(selectedProjectId, {
        ...commercial,
        website_marketing: { ...marketing, public_state: "published", prelaunch_enabled: false },
      }, user)
      await publishProjectToWebsite(selectedProjectId)
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 12, borderRadius: 12, border: `1px solid ${salesReadiness.color}`, background: salesReadiness.bg, display: "grid", gap: 10 }}>
      ...
      <div style={{ padding: 12, borderRadius: 12, border: "1px solid #e5e7eb", background: "rgba(255,255,255,0.55)", display: "grid", gap: 8 }}>
        ...
        <div style={{ color: "#475569" }}>Aqui vês rapidamente se cada canal já tem base mínima de saída pronta antes da integração externa real.</div>
        <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ padding: 12, borderRadius: 12, border: "1px solid #e5e7eb", background: "rgba(255,255,255,0.55)", display: "grid", gap: 8 }}>
        ...
        <div style={{ display: "grid", gap: 8, padding: 12, borderRadius: 12, background: "#fff", border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", color: "#64748b" }}><span>CTA: {marketing.teaser_cta_label || "-"}</span><span>•</span><span>{marketing.teaser_release_label || "-"}</span></div>
