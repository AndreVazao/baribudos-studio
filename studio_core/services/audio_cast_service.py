...
def save_audio_cast(project_id: str, payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    updated = update_json_item(PROJECTS_FILE, project_id, lambda current: {
        **current,
        "audio_cast": {
            "narrator": {
                "voice_profile_id": _safe_text(narrator.get("voice_profile_id")),
    ...
    return {"ok": True, "audio_cast": _safe_dict(updated.get("audio_cast", {})), "project": updated}
...
def list_project_character_names(project_id: str) -> List[str]:
    ...
    for item in runtime_chars:
        name = _safe_text(item.get("name")) if isinstance(item, dict) else _safe_text(item)
        if name:
            names.add(name)
